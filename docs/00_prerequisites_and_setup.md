...
**Gemini free-tier rate limits are real** — roughly ~10 requests/min, ~250K tokens/min, and a few hundred–1500 requests/day per model (Google cut free quotas substantially in Dec 2025). This is exactly why the `@cached` + `@retry` + `@rate_limited` decorators in Phase 3 matter — they keep you under the limit during the live demo. Set `@rate_limited` conservatively for the Gemini calls, not just the market APIs. If the demo needs headroom, enabling billing on the key is cheap insurance.
